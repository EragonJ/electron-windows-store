...
function convertWithContainer (program) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', (data) => utils.log(data.toString()))
    child.stderr.on('data', (data) => utils.log(data.toString()))
    child.on('exit', () => {
...
function convertWithFileCopy (program) {
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(manifest, 'utf8', (err, data) => {
      ...
      result = result.replace(/\${identityName}/g, program.identityName)
      result = result.replace(/\${applicationId}/g, program.applicationId)
